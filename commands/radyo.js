...
const radyo = {
    ...
    trtfm: "https://radiomap.eu/tr/play/trt_radyofm",
    ntv: "https://radiomap.eu/tr/play/ntv"
...
> **Radyo Kanalları: \`alem, cnnturk, fenomen, kral, kralpop, line, joyfm, fenomenturk, show, taksimclup, eksen, açık, süperfm, slowturk, efkar, trtfm, ntv\` yada kanaldan ayrılmam için \`l!radyo bitir\` yaz.**`)
...
const connection = joinVoiceChannel({
   ...
   selfDeaf: false
    ...
    message.reply("AçıkFM Çalıyor 🎧")
        ...
        const player = createAudioPlayer();
    const resource = createAudioResource(radyo.acik)
    ...
    const resource = createAudioResource(radyo.efkar)    
     ...
     await player.play(resource);
    connection.subscribe(player);
    message.reply("Trt FM Çalıyor 🎧")
    } else if(args[0] === "trtfm") {
        const player = createAudioPlayer();
    const resource = createAudioResource(radyo.trtfm)

     await player.play(resource);
    connection.subscribe(player);
    message.reply("Ntv Çalıyor 🎧")
    } else if(args[0] === "ntv") {
        const player = createAudioPlayer();
    const resource = createAudioResource(radyo.ntv)

            getVoiceConnection(`${message.guild.id}`).disconnect();
        message.reply("**Ses kanalından ayrıldım**")
    } else {
        return message.reply(`**Doğru bir radyo kanalı yaz lütfen**
         **Radyo Kanalları: \`alem, cnnturk, fenomen, kral, kralpop, line, joyfm, fenomenturk, show, taksimclup, eksen, açık, süperfm, slowturk, efkar, trtfm, ntv\` yada kanaldan ayrılmam için \`l!radyo bitir\` yaz.**`)
    }
}
