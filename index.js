...
client.on("messageCreate", async message => {
  ...
  if (sebep) {
    message.reply("**Etiketlediğin kullanıcı şuan AFK**");
  }
});

client.on('guildMemberAdd', member => {
    member.send("Hoş geldin.Lütfen emojiye basıp rolünü almayı unutma!").catch(err => console.log(err));
})
...
message.delete()
message.channel.send(`**Hey <@${message.author.id}>, Bu Sunucuda Küfür Engel Sistemi Aktif!!! Küfür Edemezsin __Laflarına__ Dikkat Et** `)
}
}
})
client.on("messageCreate", (message) => {
...
message.delete()
message.channel.send(`**Hey <@${message.author.id}>, Bu Sunucuda Reklam Engel Sistemi Aktif!!! __Link__ Atamazsın.**`)
}
}
})
...
message.channel.send(`<@${message.author.id}> **Aleykümselam, Kardeşim Hoşgeldin.**`)
}
}
})
